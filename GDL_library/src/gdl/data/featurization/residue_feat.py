...
class ResidueFeaturizer(Featurizer):
    ...
    def process_structure(self, structure_file, process_bonds: bool = True):
        """
        ...
        :return: A tuple containing:
            ...
            - bonds_features
        """
        traj = self._get_trajectory(structure_file)
        topo = traj.topology
        seq = self._get_sequence(topo=topo)
        coords = self._get_coords(traj, topo)
        ...
        bonds, bonds_features = self._get_bonds(process_bonds)

        return FeatureData(node_features, coords, bonds, bonds_features)

    #Not implemented
    def _get_bonds(self, process_bonds: bool):
        """
    ...
    def _get_trajectory(self, structure_file):
        """
    ...
    def _get_sequence(self, topo: md.Topology):
        """
    ...
    def _get_coords(self, traj: md.Trajectory, top: md.Topology):
        """
        ...
        match self.residue_center:
            case "ca":
                return self._get_alpha_coords(traj, top)
            case "mass":
                return self._get_center_mass_coords(traj, top)
            case "mean":
                return self._get_mean_center_coords(traj, top)
            case _:
                return self._get_alpha_coords(traj, top)


    def _get_alpha_coords(self, traj: md.Trajectory, top: md.Topology):
        """
    ...
    def _get_center_mass_coords(self, traj: md.Trajectory, top: md.Topology):
        """
    ...
    def _get_mean_center_coords(self, traj: md.Trajectory, top: md.Topology):
        """
